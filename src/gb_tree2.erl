...
%%
%% - closest(K, T): returns {V}, where V is the value to the nearest
%%   or exact value of K. This method has been introduced for the
%%   purpose of supporting a consistent hash implementation.
...
%% This is a specialized version of `lookup'. It guarantees a match
%% by falling back to the last good value if no match has been found.


-spec closest(Key, Tree) -> {'value', Val} when
      Key :: term(),
      Val :: term(),
      Tree :: gb_tree().
...
closest_1(Key, {Key1, Value, nil, _}) when Key < Key1 ->
    {value, Value};
closest_1(Key, {Key1, _, Smaller, _}) when Key < Key1 ->
    closest_1(Key, Smaller);
closest_1(Key, {Key1, Value, _, nil}) when Key > Key1 ->
    {value, Value};
